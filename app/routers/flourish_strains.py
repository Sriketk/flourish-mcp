...
class Strain(SQLModel, table=True):
    __tablename__ = "flourish_strains"
...
@router.get("/flourish/strains/", response_model=StrainResponse, tags=["flourish_strains"], operation_id="get_flourish_strains")
def get_strains(
    session: SessionDep,
    offset: int = 0,
    limit: int = 50
    ...
    """
    Get a list of all flourish strains with pagination support.
...
@router.get("/flourish/strains/{strain_id}", response_model=SingleStrainResponse, tags=["flourish_strains"], operation_id="get_flourish_strain_by_id")
def get_strain_by_id(strain_id: str, session: SessionDep) -> SingleStrainResponse:
    """
    Get a single flourish strain by its ID.
...
@router.post("/flourish/strains/", response_model=SingleStrainResponse, tags=["flourish_strains"], operation_id="post_flourish_strain")
def post_strain(strain: Strain, session: SessionDep) -> SingleStrainResponse:
    """
    Create a new flourish strain.
